...
@interface MHWebView () <WKNavigationDelegate, WKUIDelegate>
...
- (void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation {
    if (self.MHQRCodeDelegate && [self.MHQRCodeDelegate respondsToSelector:@selector(webViewDidStartLoad:)]) {
        [self.MHQRCodeDelegate webViewDidStartLoad:self];
    }
}
/// 当内容开始返回时调用
- (void)webView:(WKWebView *)webView didCommitNavigation:(WKNavigation *)navigation {
    if (self.MHQRCodeDelegate && [self.MHQRCodeDelegate respondsToSelector:@selector(webView:didCommitWithURL:)]) {
        [self.MHQRCodeDelegate webView:self didCommitWithURL:self.wkWebView.URL];
    }
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    self.navigationItemTitle = self.wkWebView.title;
    if (self.MHQRCodeDelegate && [self.MHQRCodeDelegate respondsToSelector:@selector(webView:didFinishLoadWithURL:)]) {
        [self.MHQRCodeDelegate webView:self didFinishLoadWithURL:self.wkWebView.URL];
    }
...
- (void)webView:(WKWebView *)webView didFailNavigation:(WKNavigation *)navigation withError:(NSError *)error {
    if (self.MHQRCodeDelegate && [self.MHQRCodeDelegate respondsToSelector:@selector(webView:didFailLoadWithError:)]) {
        [self.MHQRCodeDelegate webView:self didFailLoadWithError:error];
    }
