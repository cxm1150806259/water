...
@implementation MHPopTool

+ (MHPopTool *)sharedInstance {
    static MHPopTool *sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedClient = [[MHPopTool alloc]init];
    });
    return sharedClient;
}
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([MHPopTool sharedInstance].shadeBackgroundType == ShadeBackgroundTypeSolid) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([MHPopTool sharedInstance].tapOutsideToDismiss) {
        [[MHPopTool sharedInstance] hideAnimated:YES withCompletionBlock:nil];
    }
}
