...
-(UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        _scrollViewMaxY = CGRectGetMaxY(_scrollView.frame);
    }
    return _scrollView;
}
...
-(void)creatItemView{
    ...
    for ( int i = 0; i < _itemsCount; i++ ) {
        CGFloat imgX = ScreenWidth/2-300/2+i*ScrollWidth;
        MHHomeActivityModel *adModel = [_adDataList objectAtIndex:i];
        ADItemView*item = [[ADItemView alloc]initWithFrame:CGRectMake(imgX,imgY,imgW,imgH)];
        item.backgroundColor = [UIColor clearColor];
        item.userInteractionEnabled = YES;
        item.index  = i;
        item.tag    = BaseTag+item.index;
//        item.imageView.image      = [UIImage imageNamed:adModel.pushImageUrl];
        [item.imageView sd_setImageWithURL:[NSURL URLWithString:adModel.pushImageUrl] placeholderImage:[UIImage imageNamed:@""]];
    ...
    cancelBtn.frame = CGRectMake(ScreenWidth/2-22, self.scrollViewMaxY + 10, 44, 44);
    [cancelBtn setImage:[UIImage imageNamed:@"home_close"] forState:UIControlStateNormal];
    [cancelBtn addTarget:self action:@selector(removeSelfFromSuperview) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:cancelBtn];
    
    //初始化pageControl
    pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(0, self.scrollViewMaxY - 20, ScreenWidth, 20)];
