//
//  MHQRScanActivateDeviceViewController.m
//  waterever
//
//  Created by qyyue on 2017/8/29.
...
#import "MHQRScanedModel.h"

@interface MHQRScanActivateDeviceViewController ()<SGQRCodeScanManagerDelegate, SGQRCodeAlbumManagerDelegate>
...
@implementation MHQRScanActivateDeviceViewController
...
- (void)QRCodeAlbumManager:(SGQRCodeAlbumManager *)albumManager didFinishPickingMediaWithResult:(NSString *)result {
    if ([result containsString:@"deviceid="]) {
        NSRange range = [result rangeOfString:@"deviceid="];
        unsigned long location = range.location + range.length;
        NSString *resultString = [result substringFromIndex:location];
        [self activateDeviceWithDeviceCode:resultString];
    }else{
        [SVProgressHUD showErrorWithStatus:@"暂未识别出扫描的二维码"];
    }
}
...
#pragma mark - - - 激活设备
-(void)activateDeviceWithDeviceCode:(NSString *)deviceCode{
    ...
    [parameters setObject:deviceCode forKey:@"device_code"];
    
    [MHNetWorkTool POSTWITHHEADER:[WATEREVERHOST stringByAppendingString:@"device/2.0/activate"] parameters:parameters success:^(id responseObject) {
        int status = [responseObject[@"status"] intValue];
        if(status == 2101){
            MHResultErrorViewController *vc = [[MHResultErrorViewController alloc]init];
            ...
            MHQRScanedModel *model = [MHQRScanedModel mj_objectWithKeyValues:responseObject[@"data"]];
            MHUserInfoTool *userInfo = [MHUserInfoTool sharedMHUserInfoTool];
            userInfo.personDeviceId = model.personDeviceId;
            [MHUserInfoTool saveUserInfo];
            
            MHResultSuccessViewController *vc = [[MHResultSuccessViewController alloc]init];
            [self.navigationController pushViewController:vc animated:YES];
            vc.navigationItem.leftBarButtonItem=[UIBarButtonItem itemWithImage:@"h5_back_normal" HighlightedImage:@"h5_back_hightlight" Target:self Action:@selector(backToRoot)];
            vc.navigationItem.rightBarButtonItem=nil;
